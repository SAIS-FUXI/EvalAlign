...
def forward(model, input, scales=None, img_sizes=None, max_split_size=None, resize_output_to_idx=0, num_prefix_token=0,
    ...
    #print("outs_multiscale",len(outs_multiscale),outs_multiscale[0].size(),outs_multiscale[1].size(),outs_multiscale[2].size())
    ...
    #print("outs_multiscale_1",len(outs_multiscale),outs_multiscale[0].size(),outs_multiscale[1].size(),outs_multiscale[2].size())
    ...
    #print("outs_multiscale_2",len(outs_multiscale),outs_multiscale[0].size(),outs_multiscale[1].size(),outs_multiscale[2].size())
    ...
    out = torch.cat([F.interpolate(outs_multiscale[i].to(torch.float32), size=output_size,
                                   mode='area').to(outs_multiscale[i].dtype).unsqueeze(1)
                     for i in range(len(outs_multiscale))], dim=1)
    #print("out",out.size())
    #raise()
    if output_shape == 'bnc':
        out = rearrange(out, 'b k c h w -> b (k h w) c')
